    ...
    <template x-if="projects.length > 0">
        <div class="flex flex-col flex-1 min-h-0 md:gap-3">
            ...
            <div class="hidden md:grid grid-cols-6 gap-4">
                ...
                <div class="bg-white rounded-lg shadow-sm border p-2 md:p-4">
                    ...
                    <div class="mt-3 h-3 bg-gray-100 rounded-full overflow-hidden flex" aria-label="Распределение задач по статусам">
                        <div class="h-full bg-amber-400 transition-all duration-300" :style="'width:' + (currentStats.total > 0 ? Math.round(currentStats.in_progress / currentStats.total * 100) : 0) + '%'"></div>
                        <div class="h-full bg-orange-400 transition-all duration-300" :style="'width:' + (currentStats.total > 0 ? Math.round(currentStats.revision / currentStats.total * 100) : 0) + '%'"></div>
                        <div class="h-full bg-green-500 transition-all duration-300" :style="'width:' + (currentStats.total > 0 ? Math.round(currentStats.done / currentStats.total * 100) : 0) + '%'"></div>
                        <div class="h-full bg-indigo-500 transition-all duration-300" :style="'width:' + (currentStats.total > 0 ? Math.round(currentStats.closed / currentStats.total * 100) : 0) + '%'"></div>
                    </div>
                    <div class="mt-3 grid grid-cols-2 gap-x-2 gap-y-1.5 text-[11px] text-gray-600" aria-label="Легенда статусов">
                        <span class="flex items-center gap-1.5"><span class="h-2 w-2 flex-shrink-0 rounded-full bg-amber-400"></span>В работе</span>
                        <span class="flex items-center gap-1.5"><span class="h-2 w-2 flex-shrink-0 rounded-full bg-orange-400"></span>Доработки</span>
                        <span class="flex items-center gap-1.5"><span class="h-2 w-2 flex-shrink-0 rounded-full bg-green-500"></span>Готово</span>
                        <span class="flex items-center gap-1.5"><span class="h-2 w-2 flex-shrink-0 rounded-full bg-indigo-500"></span>Закрыто</span>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow-sm border p-2 md:p-4">
                    <div class="text-xs text-gray-500 mb-3">Время</div>
                    <div class="space-y-2">
                        <div class="flex items-center gap-2">
                            <span class="text-xs text-gray-500 w-10">План</span>
                            <div class="flex-1 h-3 bg-gray-100 rounded overflow-hidden"><div class="h-full bg-blue-400 rounded transition-all duration-300" :style="'width:' + (currentTimeData.maxHours > 0 ? Math.round(currentTimeData.estimated / currentTimeData.maxHours * 100) : 0) + '%'"></div></div>
                            <span class="text-xs font-medium text-gray-700 min-w-[38px] text-right" x-text="currentTimeData.estimated + ' ч'"></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-xs text-gray-500 w-10">Факт</span>
                            <div class="flex-1 h-3 bg-gray-100 rounded overflow-hidden"><div class="h-full rounded transition-all duration-300" :class="currentTimeData.actual > currentTimeData.estimated && currentTimeData.estimated > 0 ? 'bg-red-400' : 'bg-green-400'" :style="'width:' + (currentTimeData.maxHours > 0 ? Math.round(currentTimeData.actual / currentTimeData.maxHours * 100) : 0) + '%'"></div></div>
                            <span class="text-xs font-medium min-w-[38px] text-right" :class="currentTimeData.actual > currentTimeData.estimated && currentTimeData.estimated > 0 ? 'text-red-600' : 'text-green-600'" x-text="currentTimeData.actual + ' ч'"></span>
                        </div>
                        <template x-if="currentTimeData.manager > 0">
                            <div class="flex items-center gap-2">
                                <span class="text-xs text-gray-500 w-10">Рук.</span>
                                <div class="flex-1 h-3 bg-gray-100 rounded overflow-hidden"><div class="h-full rounded bg-purple-400 transition-all duration-300" :style="'width:' + (currentTimeData.maxHours > 0 ? Math.round(currentTimeData.manager / currentTimeData.maxHours * 100) : 0) + '%'"></div></div>
                                <span class="text-xs font-medium text-purple-600 min-w-[38px] text-right" x-text="currentTimeData.manager + ' ч'"></span>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            <!-- Десктоп: четыре статусные колонки -->
    ...
    <div x-show="showInfoModal" role="dialog" aria-modal="true" aria-labelledby="project-stats-title"
        ...
        <div class="bg-white w-full rounded-t-2xl p-4 pb-8 space-y-3 max-h-[70vh] overflow-y-auto"
            ...
            <div class="bg-white rounded-lg border p-3">
                ...
                <div class="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-gray-600" aria-label="Легенда статусов">
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-amber-400"></span>В работе</span>
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-orange-400"></span>Доработки</span>
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-green-500"></span>Готово</span>
                    <span class="flex items-center gap-2"><span class="h-2.5 w-2.5 rounded-full bg-indigo-500"></span>Закрыто</span>
                </div>
            </div>
