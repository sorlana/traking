                ...
                <div class="relative min-w-0 min-h-[64px] sm:min-h-[90px] p-1 sm:p-1.5 <?= $day['today'] ? 'bg-blue-50 ring-2 ring-inset ring-blue-400' : ($day['weekend'] ? 'bg-gray-50' : 'bg-white') ?> <?= ($canQuickEntry && !$isFuture) ? 'cursor-context-menu' : '' ?>"
                    ...
                    <div class="space-y-1">
                        <?php foreach ($dayEntries as $entry): ?>
                            ...
                            ?>
                            <?php if ($canQuickEntry): ?>
                                <div class="group/entry flex min-w-0 items-start gap-1 rounded px-1 py-1 text-[10px] sm:text-xs leading-tight <?= $entryClass ?>">
                                    <!-- Левый столбец: чекбокс сверху, под ним иконки действий (компактно) -->
                                    <div class="flex flex-shrink-0 flex-col items-center leading-none">
                                    ...
                                    <a href="<?= url('/tasks/' . $entry['task_id']) ?>"
                                       class="block min-w-0 flex-1 pt-0.5"
