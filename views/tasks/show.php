 ...
 *
 * Структура:
 * 1. Строка с двумя кнопками: «Задача» (открывает модалку с шапкой+действиями) и «Детали» (модалка с правой панелью)
 * 2. Чат на всю оставшуюся высоту
 */
$layout = 'layouts/app';
...
<div class="flex flex-col h-full" x-data="{ modal: null, tab: 'subtasks', reassignOpen: false }">
    ...
    <div class="flex items-center gap-2 mb-3">
        ...
        <?php if ($isOverdue): ?>
            <span class="flex-shrink-0 px-2 py-0.5 bg-red-100 text-red-700 rounded text-xs font-medium">⚠</span>
        <?php endif; ?>
        <div class="flex-1"></div>
        <button @click="modal = 'details'"
                class="px-3 py-1.5 bg-white border rounded-md text-sm text-gray-700 hover:bg-gray-50 shadow-sm transition">
            Детали
        </button>
    </div>
    ...
    <!-- ===== Модалка: Задача (шапка + действия) ===== -->
    <div x-show="modal === 'task'" x-transition.opacity
            ...
            <div class="p-4">
                ...
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <?php if (($task['priority'] ?? 'medium') !== 'medium'): ?>
                        <span class="px-2 py-0.5 rounded text-xs font-medium <?= $prio['class'] ?>"><?= $prio['label'] ?></span>
                    <?php endif; ?>
                    <?php if ($isOverdue): ?>
                        <span class="px-2 py-0.5 bg-red-100 text-red-700 rounded text-xs font-medium">⚠ Просрочена</span>
                    <?php endif; ?>
                </div>
                ...
                <div class="mb-4">
                    <p class="text-sm text-gray-700 whitespace-pre-wrap"><?= e($task['description']) ?></p>
                </div>
                <?php endif; ?>
                ...
                <div class="flex items-center gap-2 flex-wrap border-t pt-4">
                    ...
                    <?php if ($canEdit && !$isClosed): ?>
                        <div class="relative">
                            <button @click="reassignOpen = !reassignOpen"
                                    class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200 transition">Переназначить</button>
                            <div x-show="reassignOpen" @click.outside="reassignOpen = false" x-transition
                                 class="absolute left-0 bottom-full mb-1 bg-white rounded-lg shadow-lg border py-1 min-w-[200px] z-50"
                                 style="display: none;">
                    ...
                    <?php if ($canEdit && $task['status_code'] === 'done' && !$isExecutor): ?>
                        <form method="POST" action="<?= url('/tasks/' . (int) $task['id'] . '/close') ?>"
                            ...
                            <?= csrf_field() ?>
                            <button type="submit" class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200 transition"
                                    <?= !$canClose['can'] ? 'disabled title="Есть незавершённые доработки"' : '' ?>>Закрыть задачу</button>
                        </form>
                    <?php endif; ?>
    ...
    <!-- ===== Модалка: Детали (правая панель с тремя вкладками) ===== -->
    <div x-show="modal === 'details'" x-transition.opacity
         class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
         @click.self="modal = null" style="display: none;">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col" @click.stop>
            ...
            <div class="flex items-center justify-between border-b px-4 pt-3">
                <nav class="flex gap-4">
                    <button @click="tab = 'subtasks'"
                            :class="tab === 'subtasks' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'"
                            class="pb-2 border-b-2 text-sm font-medium transition">
                        Доработки <span class="text-xs opacity-70"><?= count($children) ?></span>
                    </button>
                    <button @click="tab = 'info'"
                            :class="tab === 'info' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'"
                            class="pb-2 border-b-2 text-sm font-medium transition">
                        Информация
                    </button>
            ...
            <div class="p-4 overflow-y-auto flex-1">
                ...
                <div x-show="tab === 'subtasks'" x-data="{ showAddForm: false }">
                    <?php if ($canEdit): ?>
                        <div class="mb-3">
                            <button x-show="!showAddForm" @click="showAddForm = true; $nextTick(() => $refs.subtaskInput2.focus())"
                            ...
                            <form x-show="showAddForm" x-transition
                                ...
                                <input type="text" name="title" x-ref="subtaskInput2" required
